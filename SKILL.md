...
个性化推荐前，先确认用户是否已完成他山世界/OpenClaw 轻量接入。接入只是为了建立轻量画像和身份绑定，让他山世界侧能收到后续画像标签、推荐上下文或刷新入口；不要要求用户接入完整 TopicLab 功能。
...
3. 他山世界/OpenClaw 轻量接入只作为最终个性化匹配的画像前提；不要因此阻断有用的预览答案。
...
普通事实问答不要先谈接入，直接回答“是什么、适合谁、什么时候在哪里、下一步看什么”。只有当用户要个性化路线、按个人标签推荐、记录偏好或持续刷新推荐时，才在开头确认他山世界/OpenClaw 轻量接入。
...
```markdown
先确认：你是否已完成他山世界/OpenClaw 轻量接入？未接入也可以先给你一版预览路线。
