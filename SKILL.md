...
- 新会话、新安装或发现本地会话不可用时，加载 `references/tashan_world_bridge.md`，默认先完成轻量接入：有 `?key=...` 的专属 skill 链接就按 `topiclab session ensure` 处理；只有不带 key 的通用入口且本地没有可用 bind key 时，只允许做一次访客 bootstrap。
