...
1. 已完成他山世界/OpenClaw 轻量接入：使用用户画像标签做最终匹配。
2. 未接入：仍给预览路线，但提醒完成轻量接入后可保存画像标签和推荐上下文。
3. 日期优先于主题。用户只有一天时，不跨天推荐。
4. 地图成本优先于微弱主题匹配。相邻区域的 80 分活动通常优于远处的 85 分活动。
5. 完整/通用行程默认包含一个新生论坛作为认知主线；除非用户明确说不想听报告、只有碎片时间，或低能量状态不适合。
...
如果没有完全匹配的新生论坛，不要直接删掉论坛段；先找相邻主题的新生论坛，并说明“这里放它是为了建立当天主线”。
