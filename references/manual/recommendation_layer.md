...
1. 如果后台已有用户画像标签，静默用于最终匹配。
2. 如果用户没有给足画像，不谈接入状态，直接追问最影响推荐的参会信息。
3. 日期优先于主题。用户只有一天时，不跨天推荐。
4. 地图成本优先于微弱主题匹配。相邻区域的 80 分活动通常优于远处的 85 分活动。
5. 完整/通用/半天路线默认包含一个新生论坛作为认知主线；低能量不是删除论坛的理由，只是把论坛改成短听、可提前离场、只听最相关一场，或作为可选锚点。
6. 每条路线至少包含一个缓冲点：热带雨林、探索空间、青春舞台、星空露营或餐饮/休息。
7. 不要连续安排三个高强度论坛。
8. 对“第一次来”用户，先给一个低门槛入口，再给一个新生论坛主线，再接探索空间/团聚/夜间缓冲。
9. 行程不是候选清单。主时间线必须无重叠；同一时段的其他好活动只能放备选。
10. 长时段活动要区分官方时段和建议停留窗口。不能把多个 09:00-15:30 活动同时塞进路线。
11. 地点精度优先。活动表只有总场馆时说明需现场复核，不补写不存在的楼层或厅。
12. 不要无故安排晨读、晨跑等早晨活动；它们只有在画像或请求明确相关时才作为低压力入口。

## 经验型行程骨架

- 低门槛入口：先让用户熟悉场地和人，适合热带雨林、探索空间或青年团聚。
- 认知主线：安排一个与用户兴趣最相关的新生论坛，作为当天“为什么来”的主活动。
- 实践连接：用探索空间、工作坊、团聚或文章线索承接论坛主题，让用户能看见项目或找到人。
- 恢复收尾：用热带雨林、青春舞台、星空露营、餐饮或散步降低强度。

如果没有完全匹配的新生论坛，不要直接删掉论坛段；先找相邻主题的新生论坛，并说明“这里放它是为了建立当天主线”。只有用户明确拒绝听报告，或抵达时间完全错过论坛，才把新生论坛改成回看/文章线索或次日候选。
